$(function() {
    ...
    function createItemFilm( filmData ) {
        ...
        filmItem.innerHTML = '<div class="container"><div class="row"><div class="col-xl-2"><img class="film-item-poster" src="' + filmData.imageLink + '"></div><div class="col-xl-6"><div class="film-item-info"><div class="movie-title">' + filmData.name.rus + '</div><div class="movie-title">' + filmData.name.eng + '</div><div class="movie-genre">' + filmData.genre + '</div></div></div><div class="col-xl-4 px-0"><div class="film-item-rating"><span class="rating-value ' + getClassStatusFilm(filmData.rating_kp.ratingValue) + '">' + filmData.rating_kp.ratingValue + '</span><span class="rating-count">' + filmData.rating_kp.ratingCount + '</span><span class="rating-title">КиноПоиск</span></div><div class="film-item-rating"><span class="rating-value ' + getClassStatusFilm(filmData.rating_IMDb.ratingValue) + '">' + filmData.rating_IMDb.ratingValue + '</span><span class="rating-count">' + filmData.rating_IMDb.ratingCount + '</span><span class="rating-title">IMDb</span></div></div></div></div>';
        return filmItem;
    }
